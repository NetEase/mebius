...
多个工程根据diff的代码指定类、方法来分析影响调用链，分析结果根据url进行分组。
...
List<SpecifyParam> specifyParams = new ArrayList<>();
SpecifyParam specifyParam = new SpecifyParam();
specifyParam.setPkgName(pkgName);
specifyParam.setClassName(className);
specifyParam.setMethodName(methodName);
specifyParams.add(specifyParam);

try{
  List<MethodsCallResult> results = CodeAnalyze.analyzeWithSpecify(projectParams, specifyParams, null);
} catch (Exception e) {
  e.printStackTrace();
}
```
